...
public class ReadingEmail extends ScenarioSteps {
	...
	public void check_if_the_receive_mail_is_correct_when_you_make_a_new_vacation_request(String lastName,String startDate,String endDate,String type) throws IOException{	
		...
		for(i=0;i<list.size();i++){
			...
			if(a.contentEquals(c) && subject.contentEquals(tools.userEmailSubject())) found=true;
		}
	...
	@Step
	public void check_if_dm_receives_the_correct_email_when_somebody_makes_a_new_vacation_request(String lastName,String startDate,String endDate,String type){
	...
	for(i=0;i<list.size();i++){
		...
		String msg = list.get(i).getcontent();
		String c = msg.replaceAll("\\s", "");
		if(templateDM.contentEquals(c) && subject.contentEquals(tools.DMMailSubject())) found=true;
	}
	assertTrue("The email from user not found", found);
	}
	@Step
	public void check_if_the_received_mail_is_correct_when_you_has_approved_request(String lastName,String startDate,String endDate,String type){
		...
		for(i=0;i<list.size();i++){
			...
			String msg = list.get(i).getcontent();
			String c = msg.replaceAll("\\s", "");
			if(a.contentEquals(c) && subject.contentEquals(tools.userMailSubjectWhenRequestApproved())) found=true;
		}
		assertTrue("The email from user not found", found);
	}
	@Step
	public void check_if_the_received_mail_is_correct_when_you_has_rejected_request(String lastName,String startDate,String endDate,String type){
		...
		for(i=0;i<list.size();i++){
			...
			String msg = list.get(i).getcontent();
			String c = msg.replaceAll("\\s", "");
			if(a.contentEquals(c) && subject.contentEquals(tools.userEmailSubjectWhenRequestRejected())) found=true;
		}
		assertTrue("The email from user not found", found);
	...
	@Step
	public void check_if_dm_receives_the_correct_email_when_approve_request(String lastName,String startDate,String endDate,String type){
	...
	for(i=0;i<list.size();i++){
		...
		String msg = list.get(i).getcontent();
		String c = msg.replaceAll("\\s", "");
		if(templateDM.contentEquals(c) && subject.contentEquals(tools.DMMailSubjectWhenRequestApproved())) found=true;
	}
	assertTrue("The email from user not found", found);
	...
	@Step
	public void check_if_dm_receives_the_correct_email_when_reject_request(String lastName,String startDate,String endDate,String type){
		...
		for(i=0;i<list.size();i++){
			...
			String msg = list.get(i).getcontent();
			String c = msg.replaceAll("\\s", "");
			if(templateDM.contentEquals(c) && subject.contentEquals(tools.DMEmailSubjectWhenRequestRejected())) found=true;
		}
		assertTrue("The email from user not found", found);
