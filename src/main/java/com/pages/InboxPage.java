...
public class InboxPage extends PageObject {
	...
	public void selectFirstRequest() {
		WebElement check = tableWithRequests
				.findElement(By.cssSelector("input[name='_evovacation_WAR_EvoVacationportlet_rowIds']"));
		check.click();
	}
	...
	public void selectRequestsToApproveOrReject(String emplyeeName, String startDate, String endDate, String type) {
		...
		while (i < nr) {
			...
			if (listOfNames.get(i).getText().compareTo(emplyeeName) == 0
					&& listOfStartDates.get(i).getText().compareTo(startDate) == 0
					&& listOfEndDates.get(i).getText().compareTo(endDate) == 0
					&& listOfTypes.get(i).getText().compareTo(type) == 0) {
