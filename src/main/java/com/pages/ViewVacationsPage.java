...
public class ViewVacationsPage extends PageObject {
	...
	public boolean check_if_the_desired_request_was_approved_or_rejected(String employeeName, String startDate,
		...
		while (i < nr) {
			...
			if (namesFromTheTable.get(i).getText().compareTo(employeeName) == 0
					&& startDatesFromTheTable.get(i).getText().compareTo(startDate) == 0
					&& endDatesFromTheTable.get(i).getText().compareTo(endDate) == 0
					&& statusFromTheTable.get(i).getText().compareTo(status) == 0
					&& typesFromTheTable.get(i).getText().compareTo(type) == 0) {
				isCorrect = true;
				break;
			}
			i++;
